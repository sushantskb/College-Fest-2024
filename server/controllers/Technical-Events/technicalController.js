...
const TechnicalEvents = require("../../models/otherEvents/events");
...
exports.initPage = async (req, res) => {
  // all tech -> 3 types
  try {
    return res.render("technical_events", {
      loader_title_1: "Here",
      loader_title_2: "Comes",
      loader_title_3: "Technical",
    });
  } catch (error) {
    console.log(error);
  }
...
exports.secondPage = async (req, res) => {
  ...
  try {
    ...
    let eventsData = await TechnicalEvents.find({
      category: "technical",
    ...
    return res.render("technical-details", {
      eventsData,
      type,
      loader_title_1: "Here",
      loader_title_2: "Comes",
    ...
    console.log(error);
    return res.redirect("/events/technical");
  }
};
...
exports.showPage = async (req, res) => {
  ...
  const type = req.params.type;
  try {
    let eventData = await TechnicalEvents.find({
      category: "technical",
      ename: eventName,
    });
    ...
    if (eventData.gender === "female" && userData.gender === "male") {
      ...
      return res.redirect(`/events/technical/${type}`);
    }
    ...
    if (eventData.maxSize == 1) {
      // solo
      return res.render("tech_solo_register", {
      ...
      // doubles
      return res.render("tech_duo_register", {
        csrfToken: req.csrfToken(),
        type,
        userData,
        infoErrorObj,
    ...
    return res.render("tech_group_register", {
      csrfToken: req.csrfToken(),
      userData,
      eventData,
      type,
    ...
    console.log("here");
    console.log(error);
    return res.render(`/events/technical/${type}`);
  }
};

exports.registerUser = async (req, res) => {
  try {
    const eventData = await TechnicalEvents.findOne({
    ...
    if (eventData.maxSize == 1) {
      ...
      if (temp2) {
        ...
        return res.redirect(
          `/events/technical/${req.params.type}/${req.params.event}`
        );
      } else {
